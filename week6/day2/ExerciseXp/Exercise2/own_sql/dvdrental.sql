...
FROM film
WHERE lower(title) = 'blindness gun' ;
...
SELECT film_id, title, description, length, rental_rate
FROM film
WHERE lower(title) like 'bl%';

-- Write a query which will find the 10 cheapest movies.
SELECT film_id, title, rating
FROM film ORDER BY rental_rate ASC LIMIT 10;

-- Not satisfied with the results. 
-- Write a query which will find the next 10 cheapest movies.
SELECT film_id, title, rating
FROM film ORDER BY rental_rate ASC offset 10 LIMIT 10;
...
-- Write a query to get the customer’s id, names (first and last), the amount
-- the date of payment ordered by the id of the staff member who sold them the dvd.

SELECT customer.customer_id , CONCAT(customer.first_name, ' ', customer.last_name) AS "Full name" ,
